...
def match_with_gaps(my_word, other_word):
    ...
    returns: boolean, True if all the actual letters of my_word match the 
        ...
        False otherwise: 
    '''
    #missing: match_with_gaps("a_ ple", "apple") gives False  
    ...
    for i in range(len(my_word)):
            if my_word[i]=='_':
                continue
            elif my_word[i]!=other_words[i]:
                return False
    return True

def position_check(word1,temp1,lst1,j):
    '''
    word1: string with _ characters, current guess of secret word
    temp1: word whose similarity to word1 is being found
    lst1: list of indices of guessed letters
    j: counter for lst1
    returns: True, if the all the letters at the corresponding indices in in word1 and
             temp1 are the same
             False, if even a single letter is different

    '''
    if j==len(lst1):
        return True
    else:
        return word1[lst1[j]]==temp1[lst1[j]] and position_check(word1,temp1,lst1,j+1)
...
def show_possible_matches(my_word):
    ...
    for temp in wordlist:
        if len(temp)<len(word):
            continue
        elif len(temp)==len(word) and position_check(word,temp,lst,1):
            print(temp,end=" ")
